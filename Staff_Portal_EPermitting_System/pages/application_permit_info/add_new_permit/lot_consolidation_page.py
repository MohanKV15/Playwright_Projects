...
class LotConsolidationPage(GeneralInformationPage):
    def __init__(self, page):
        ...
        # Use a more specific locator for Direction to avoid ambiguity
    ...
    def fill_general_information(self, data: dict):
        """
        Fills the General Information section for Lot Consolidation.
        """
        self._wait_for_loader()
        
        # 1. Job # / Project Name
        if self.job_number_input.is_visible(timeout=2000):
            self.job_number_input.fill(data.get("project_name", "LOT-CON-AUTO"))
        ...
        # 4. Case Manager (Self-healing: skip if not visible)
        try:
        ...
        # 5. Inspector (Self-healing: skip if not visible)
        try:
    ...
    def fill_location_information(self, data: dict):
        ...
        # 2. Milepost Start (Stable pattern from DrivewayPage)
        try:
        ...
        # 4. Direction (Standard robust selection)
    ...
    def create_lot_consolidation_permit(self, data: dict):
        """
