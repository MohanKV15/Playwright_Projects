...
class StreetIntersectionPage(GeneralInformationPage):
    ...
    def fill_general_information(self, data: dict):
        """
        Fills the General Information section for Street Intersection.
        """
        self._wait_for_loader()
        
        # 1. Job # / Project Name
        if self.job_number_input.is_visible(timeout=2000):
            self.job_number_input.fill(data.get("project_name", "STR-INT-AUTO"))
        ...
        # 4. Permit Sub Type
        try:
            if self.permit_sub_type_dropdown.is_visible(timeout=2000):
    ...
    def fill_location_information(self, data: dict):
        ...
        # 2. Milepost Start (Stable pattern)
        try:
