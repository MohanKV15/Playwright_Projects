...
class PaymentPage:
    ...
    def submit_payment(self):
        ...
        # Payment gateway can take time. Wait for the redirect back to the portal's success page.
        self.page.wait_for_url(re.compile(r"CASuccessPageFull", re.I), timeout=120000)

    # -------------------------------------------------------------
    # Verify payment success and return home
    # -------------------------------------------------------------
    def verify_payment_success(self):
        
        # Verify the success page loaded with the expected text
        self.page.get_by_text(re.compile(r"Successful|Return Home", re.I)).first.wait_for(state="visible", timeout=15000)
        print("[INFO] Payment Success Verified!")
        
        # Click the Return Home button
        return_home_btn = self.page.locator("text='Return Home'")
        return_home_btn.wait_for(state="visible", timeout=5000)
        return_home_btn.click()
        # print("[INFO] Clicked Return Home button!")