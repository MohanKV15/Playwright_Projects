...
from NJDOT_EPermitting_Customer_Portal.pages.submit_application.lot_consolidation_subdivision_page import (
    LotConsolidationSubdivisionPage,
)
...
faker = Faker()
logger = logging.getLogger("test_lot_consolidation_subdivision")
...
def test_lot_consolidation_subdivision(authenticated_page):
    script_name = Path(__file__).stem
    dashboard = DashboardPage(authenticated_page, script_name=script_name)
    dashboard.click_submit_application()

    page = LotConsolidationSubdivisionPage(authenticated_page, script_name=script_name)
    page.assert_applications_page_loaded()

    # Select the application entry and start
    page.select_lot_consolidation_subdivision()
    page.click_apply_button()
    page.assert_page_loaded()
    ...
    # Permit information: choose Lot Consolidation
    page.choose_permit_type("Lot Consolidation")

    # Upload required attachments
    page.upload_required_attachments()

    # Acknowledgement and remaining required fields
    ...
    logger.info("Lot Consolidation/Subdivision flow completed.")