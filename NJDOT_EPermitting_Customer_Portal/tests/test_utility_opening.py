...
from NJDOT_EPermitting_Customer_Portal.pages.submit_application.utility_opening_page import UtilityOpeningPage
...
faker = Faker()
logger = logging.getLogger("test_utility_opening")
...
def test_utility_opening(authenticated_page):
    script_name = Path(__file__).stem
    dashboard = DashboardPage(authenticated_page, script_name=script_name)
    dashboard.click_submit_application()

    page = UtilityOpeningPage(authenticated_page, script_name=script_name)
    page.assert_applications_page_loaded()

    # Select Utility Opening application
    page.select_utility_opening()
    page.click_apply_button()
    page.assert_utility_opening_page_loaded()
    ...
    page.upload_utility_opening_attachments()
    ...
    logger.info("Utility application flow completed.") 